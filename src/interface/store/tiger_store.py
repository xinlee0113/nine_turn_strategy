...
class TigerStore(DataStoreBase):
    ...
    def get_historical_data(self, symbol: str, start_time: datetime, end_time: datetime,
                            period: str = TimeInterval.ONE_MINUTE.value) -> pd.DataFrame:
        """获取历史数据
        ...
        if period == TimeInterval.ONE_MINUTE.value:
            ...
            if date_diff > MAX_1MIN_DATA_DAYS:
                self.logger.warning(
                    f"1分钟K线数据只能获取最近{MAX_1MIN_DATA_DAYS}天的数据，当前请求时间范围：{date_diff}天")
