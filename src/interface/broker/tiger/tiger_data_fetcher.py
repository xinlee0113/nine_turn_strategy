...
class DataFetcher:
    def __init__(self, config_path, private_key_path, cache_dir):
        ...
        # 初始化API客户端
        try:
            logger.info(f"正在初始化API客户端，配置文件: {config_path}, 私钥文件: {private_key_path}")
            ...
            with open(config_path, 'r') as f:
                ...
                logger.info(f"配置文件内容: {config_content}")
            
            # 读取私钥文件
            with open(private_key_path, 'r') as f:
                key_content = f.read()
                logger.info(f"私钥文件内容: {key_content[:100]}...")  # 只打印前100个字符
            
            self.tiger_client_config = TigerOpenClientConfig(sandbox_debug=False, props_path=config_path)
            logger.info("已创建TigerOpenClientConfig")
            
            logger.info(f"正在读取私钥文件: {private_key_path}")
    ...
    def get_bar_data(self, symbol, period='1m', begin_time=None, end_time=None, use_cache=True):
        ...
        logger.info(f"开始获取数据: {symbol}, 周期: {period}")
        logger.info(f"时间范围: {begin_time} 到 {end_time}")
        ...
        logger.info(f"转换后的Tiger API周期: {tiger_period}")
        ...
        logger.info(f"数据分段信息: 总天数={total_days}, 每段最大天数={max_days_per_request}, 分段数={segment_count}")
        ...
        for i in range(segment_count):
            ...
            logger.info(f"获取第 {i+1}/{segment_count} 段数据:")
            logger.info(f"时间范围: {current_begin} 到 {current_end}")
            logger.info(f"时间戳: {begin_timestamp} 到 {end_timestamp}")
            ...
            for stock_code in stock_symbols:
                try:
                    ...
                    if isinstance(bars, pd.DataFrame) and not bars.empty:
                        logger.info(f"成功获取数据: {len(bars)} 条记录")
                        df = bars.copy()
        ...
        logger.info(f"成功获取所有数据: {len(combined_df)} 条记录")
        logger.info(f"时间范围: {combined_df.index.min()} 到 {combined_df.index.max()}")
