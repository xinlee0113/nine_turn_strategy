    ...
    def get_historical_data(self, symbol: str, start_date: datetime, 
        ...
        try:
            ...
            while current_start < end_date:
                ...
                # 转换为时间戳 (ms)
                begin_time_ms = int(current_start.timestamp() * 1000)
                # 结束时间稍微延长一点点，确保包含边界
                end_time_ms = int(current_end.timestamp() * 1000) 

                self.logger.info(f"Fetching data chunk for {symbol} from {current_start} to {current_end}")
                
                try:
                    # 获取当前块的数据, Tiger限制最多1000条，如果块太大仍可能不完整
                    # 但分块获取增加了获取完整数据的可能性
                    bars_chunk = self._api_client.get_bars(symbols=[symbol],
                                                           begin_time=begin_time_ms,
                                                           end_time=end_time_ms,
                                                           period=period,
                                                           limit=2000) # 尝试请求稍大的limit
                    
                    if bars_chunk is not None and not bars_chunk.empty:
                        all_data.append(bars_chunk)
                        self.logger.info(f"Fetched {len(bars_chunk)} records for the chunk.")
                    else:
                        self.logger.info("No data returned for this chunk.")

                except Exception as chunk_error:
                     self.logger.error(f"Error fetching chunk {current_start} to {current_end}: {chunk_error}")
                     #可以选择跳过这个块或停止，这里选择继续尝试下一个块

                # 移动到下一个块的开始时间
                current_start += chunk_delta
                
                # 短暂休眠避免API频率限制
                time.sleep(0.5) # 增加休眠时间

            if not all_data:
                self.logger.warning(f"No historical data found for {symbol} in the given period after chunking.")
                return pd.DataFrame()
