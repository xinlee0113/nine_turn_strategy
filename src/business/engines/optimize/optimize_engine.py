...
class BaseOptimizer(ABC):
    ...
    def evaluate(self, strategy_class, data: pd.DataFrame,
    ...
    def optimize_strategy_params(self,
        ...
        if self.optimize_metrics == 'return':
            sorted_results = sorted(optimization_results, key=lambda x: (x['total_return'], x['sharpe_ratio']),
                                    reverse=True)
        elif self.optimize_metrics == 'sharpe_ratio':
            sorted_results = sorted(optimization_results, key=lambda x: (x['sharpe_ratio'], x['total_return']),
                                    reverse=True)
        elif self.optimize_metrics == 'sortino_ratio':
            sorted_results = sorted(optimization_results, key=lambda x: (x['sortino_ratio'], x['total_return']),
                                    reverse=True)
        else:
            # 默认使用夏普比率
            sorted_results = sorted(optimization_results, key=lambda x: (x['sharpe_ratio'], x['total_return']),
