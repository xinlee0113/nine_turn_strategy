        ...
        if current_position > 0:  # 多头持仓
            ...
            if not np.isnan(magic_nine_sell) and magic_nine_sell >= magic_count:
                ...
                if macd_confirm or kdj_confirm:
                    signal = -1
                    self.logger.info(f"生成卖出信号: 多头平仓 (MagicNine卖出计数={magic_nine_sell}, MACD柱状图={macd_histo:.2f}, KDJ_J={kdj_j:.1f})")
                else:
            ...
            if not np.isnan(magic_nine_buy) and magic_nine_buy >= magic_count:
                ...
                if macd_confirm or kdj_confirm:
                    signal = 1
                    self.logger.info(f"生成买入信号: 空头平仓 (MagicNine买入计数={magic_nine_buy}, MACD柱状图={macd_histo:.2f}, KDJ_J={kdj_j:.1f})")
                else:
            ...
            if not np.isnan(magic_nine_buy) and magic_nine_buy >= magic_count:
                ...
                if trend_up and rsi_ok and (macd_confirm or kdj_confirm):
                    signal = 1
                    self.logger.info(f"生成买入信号: 新建多头 (MagicNine买入计数={magic_nine_buy}, RSI={rsi:.1f}, MACD柱状图={macd_histo:.2f}, KDJ_K={kdj_k:.1f})")
                else:
            ...
            # 空头信号条件：卖出计数达标且EMA20<EMA50（下降趋势）且RSI不超卖
                ...
                if trend_down and rsi_ok and (macd_confirm or kdj_confirm):
                    signal = -1
                    self.logger.info(f"生成卖出信号: 新建空头 (MagicNine卖出计数={magic_nine_sell}, RSI={rsi:.1f}, MACD柱状图={macd_histo:.2f}, KDJ_K={kdj_k:.1f})")
                else:
                    self.logger.info(f"卖出信号被过滤: 确认指标不支持 (趋势={trend_down}, RSI={rsi:.1f}, MACD={macd_confirm}, KDJ={kdj_confirm})")
