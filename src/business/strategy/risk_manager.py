    ...
    def check_risk_management(self, data: Dict[str, Any]) -> Tuple[int, Optional[float], Optional[bool]]:
        ...
        if current_position > 0:
            ...
            if buy_price > 0:
                ...
                if profit_pct >= self.profit_target_pct:
                    self.logger.info(f"达到多头利润目标! 价格: {current_price:.2f}, 买入价: {buy_price:.2f}, 利润: {profit_pct:.2f}%")
                    action = -1  # 平多头
                    return action, None, None
        ...
        # 空头仓位风险管理
            ...
            if buy_price > 0:
                profit_pct = (buy_price / current_price - 1) * 100
                if profit_pct >= self.profit_target_pct:
                    self.logger.info(f"达到空头利润目标! 价格: {current_price:.2f}, 卖出价: {buy_price:.2f}, 利润: {profit_pct:.2f}%")
                    action = 1  # 平空头
                    return action, None, None
