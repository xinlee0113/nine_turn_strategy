...
class MagicNineStrategyWithSmartStopLoss(bt.Strategy):
    """神奇九转交易策略 - 智能止损版本，包含波动性自适应、市场环境感知和时间衰减等功能"""
    params = (
    ...
    def _set_initial_stop_loss(self):
        ...
        logger.info(f'{self.data.datetime.datetime(0).isoformat()} 设置智能初始止损: {self.stop_loss_price:.2f} '
        ...
        if self.p.trailing_stop and current_price > self.highest_price:
            ...
            if profit_pct >= self.p.min_profit_pct:
                ...
                if new_stop_loss > self.stop_loss_price:
                    ...
                    logger.info(f'{self.data.datetime.datetime(0).isoformat()} 更新智能追踪止损: {old_stop_loss:.2f} -> {new_stop_loss:.2f} '
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.stop_loss_price is not None and current_price <= self.stop_loss_price:
                ...
                logger.info(f'{self.data.datetime.datetime(0).isoformat()} 智能止损触发! 盈亏: {loss_pct:.2f}%, '
            ...
            # 时间止损 - 持仓超过7天且没有足够利润
            # 约每天78个5分钟K线，7天大约为546个K线
            if self.bars_since_entry > 546:
                # 计算当前盈利百分比
                profit_pct = (current_price / self.buy_price - 1) * 100
                # 如果持仓7天后盈利低于2%，考虑退出
                if profit_pct < 2.0:
                    logger.info(f'{self.data.datetime.datetime(0).isoformat()} 时间止损触发! 持仓时间过长且收益低: {profit_pct:.2f}%, '
