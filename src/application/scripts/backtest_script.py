...
class BacktestScript:
    ...
    def run(self, symbol="AAPL", start_date=None, end_date=None, period=TimeInterval.ONE_MINUTE.value):
            ...
            if start_date is None:
                ...
                if isinstance(start_date, datetime) and isinstance(end_date, datetime):
                    ...
                    if days_diff > MAX_1MIN_DATA_DAYS:
                        self.logger.warning(f"1分钟K线数据请求时间范围超过{MAX_1MIN_DATA_DAYS}天 ({days_diff}天)，将被截断为{MAX_1MIN_DATA_DAYS}天")
                        start_date = end_date - timedelta(days=MAX_1MIN_DATA_DAYS)
        else:
        ...
        # 记录回测时间范围
        self.logger.info(f"回测时间范围: {start_date.strftime('%Y-%m-%d %H:%M:%S')} - {end_date.strftime('%Y-%m-%d %H:%M:%S')}")
        # 计算时间跨度 - 确保时区一致
        ...
        # 4. 创建数据源
        self.logger.info(f"4. 创建数据源: {symbol}, {start_date.strftime('%Y-%m-%d')} - {end_date.strftime('%Y-%m-%d')}, 周期: {period}")
        try:
            # 创建PandasData实例
