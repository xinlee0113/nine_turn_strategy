...
def parse_args():
    ...
    parser.add_argument('--days', type=int, default=MAX_1MIN_DATA_DAYS, help=f'回测过去的天数。对于1分钟K线，最大为{MAX_1MIN_DATA_DAYS}天')
    ...
    try:
        ...
        if args.mode == 'backtest':
            ...
            if args.interval == TimeInterval.ONE_MINUTE.value and days > MAX_1MIN_DATA_DAYS:
                logger.warning(f"1分钟K线数据只能获取最近{MAX_1MIN_DATA_DAYS}天的数据，但指定了{days}天，自动调整为{MAX_1MIN_DATA_DAYS}天")
                days = MAX_1MIN_DATA_DAYS
                
            start_date = end_date - timedelta(days=days)
            
            logger.info(f"回测参数: 标的={args.symbol}, 周期={args.interval}, 时间范围=过去{days}天 ({start_date.strftime('%Y-%m-%d')} 至 {end_date.strftime('%Y-%m-%d')})")
